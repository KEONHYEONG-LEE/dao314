    ...
    <div className="flex flex-col min-h-screen bg-white pb-20">
        ...
        <div className="flex gap-4 px-4">
          {CATEGORIES.map(cat => (
            <button 
              ...
              className={`flex flex-col items-center min-w-[56px] transition-all ${activeCategory === cat.id ? 'opacity-100 scale-105' : 'opacity-40 hover:opacity-70'}`}
            >
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl transition-colors ${activeCategory === cat.id ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}>
                {cat.icon}
              </div>
      ...
      <main className="divide-y divide-gray-100">
        {loading ? (
          ...
          news.map((item) => (
            <article key={item.id} className="flex items-center gap-4 p-4 active:bg-gray-50 cursor-pointer" onClick={() => { setSelectedNews(item); setSummary(""); }}>
